...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.distance_fc2 = nn.Sequential(
                nn.Linear(args.max_k  * 2, mid_size),
                nn.Tanh(),
    ...
    def distance_base(self, knn_dists, network_probs, label_counts, tgt_index, knn_key, target, last_hidden, update_num):
        knn_lambda = None
        ...
        if self.training:
            ...
            noise_var = 0.01
            e = 1000
            random_rate = random_rate * math.exp((-update_num)/e)
        ...
        return probs, knn_lambda, tgt_index 
    ...
    def forward(
        ...
        if self.use_knn_datastore:
            ...
            knn_key = knn_search_result['knn_key']
            ...
            if self.knn_lambda_type == "trainable" and self.knn_k_type == 'trainable':
                network_probs = utils.softmax(x, dim=-1, onnx_trace=self.onnx_trace)  # [batch, seq len, vocab size]
                probs, knn_lambda, tgt_index = self.distance_base(knn_dists, network_probs, label_counts, tgt_index, knn_key, target, last_hidden, update_num)
                probs = scatter(src=probs, dim_size=network_probs.size(-1), index=tgt_index, dim=-1)
                decode_result = {'prob':probs}
